...
class NasdaqInstitutionalProductionEngine(QCAlgorithm):
    """
    ...
    def Initialize(self):
        ...
        self.SetCash(50_000_000)

        self.benchmark = self.AddEquity("QQQ", Resolution.Daily).Symbol
        self.SetBenchmark(self.benchmark)

        self.qqq_ma200 = self.SMA(self.benchmark, 200, Resolution.Daily)
        self.qqq_ma_window = RollingWindow[IndicatorDataPoint](2)
        self.qqq_ma200.Updated += lambda sender, updated: self.qqq_ma_window.Add(updated)

        self.UniverseSettings.Resolution = Resolution.Daily
        self.UniverseSettings.SlippageModel = VolumeShareSlippageModel(0.10)
        self.UniverseSettings.FeeModel = InteractiveBrokersFeeModel()

        self.AddUniverse(self.CoarseSelectionFunction, self.FineSelectionFunction)
        self.Schedule.On(self.DateRules.MonthStart(), self.TimeRules.AfterMarketOpen("QQQ", 30), self.RebalanceStrategy)
        ...
        for symbol in added_symbols:
            ...
            if symbol not in self.indicators:
                self.indicators[symbol] = {
                    'price_window': RollingWindow[float](self.MOM_WINDOW + 5),
